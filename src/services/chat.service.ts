...
export class ChatService {
  ...
  // Create a new chat
  static async createChat(token: string, data: CreateChatRequest): Promise<Chat> {
    try {
      ...
      console.log('Create chat response headers:', Object.fromEntries(response.headers.entries())); // Debug log
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          throw new Error('Forbidden. You do not have permission to create this chat.');
        }
  ...
  static async sendMessage(token: string, chatId: string, data: SendMessageRequest): Promise<Message> {
    try {
  ...
  static async getChatMessages(token: string, chatId: string): Promise<Message[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/chats/${chatId}/messages`, {
        method: 'GET',
        headers: ChatService.getAuthHeaders(token),
...
export const chatService = {
  ...
  getChatById: (token: string, chatId: string) => ChatService.getChatById(token, chatId),
  createChat: (token: string, data: CreateChatRequest) => ChatService.createChat(token, data),
  sendMessage: (token: string, chatId: string, data: SendMessageRequest) => ChatService.sendMessage(token, chatId, data),
  getChatMessages: (token: string, chatId: string) => ChatService.getChatMessages(token, chatId),
  markChatAsRead: (token: string, chatId: string) => ChatService.markChatAsRead(token, chatId),
  deleteChat: (token: string, chatId: string) => ChatService.deleteChat(token, chatId),
}; 